...
import UIKit
import Firebase
class FurniturePost: SafeJsonObject {
    var item: String?
...
class FurnitureFeedController: UICollectionViewController, UICollectionViewDelegateFlowLayout {
    
    var posts = [FurniturePost]()
    ...
    override func viewDidLoad() {
        ...
        let url = URL(string: AppConfig.furniture_feed)
        URLSession.shared.dataTask(with: url!) { (data, response, error) in
            ...
            do {
                ...
                if let postsArray = json["furniture"] as? [[String: String]] {
                    
                    self.posts = [FurniturePost]()
                    
                    for postDictionary in postsArray {
                        let post = FurniturePost()
        ...
        collectionView?.register(FurnitureFeedCell.self, forCellWithReuseIdentifier: cellId)
        setupNavBarButtons()
    ...
    func setupNavBarButtons() {
        ...
        let logo = UIBarButtonItem(image: UIImage(named: "logosmall")?.withRenderingMode(.alwaysOriginal), style: .plain, target: self, action: #selector(handleMore))
        let navItem = UINavigationItem(title: "Furniture Ideas")
    ...
    override func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let FurnitureFeedCell = collectionView.dequeueReusableCell(withReuseIdentifier: cellId, for: indexPath) as! FurnitureFeedCell
        
        FurnitureFeedCell.post = posts[(indexPath as NSIndexPath).item]
        FurnitureFeedCell.feedController = self
        
        return FurnitureFeedCell
    }
...
class FurnitureFeedCell: UICollectionViewCell {
    var ref: FIRDatabaseReference!
    var feedController: FurnitureFeedController?
    
    var post: FurniturePost? {
        didSet {
    ...
    let likeButton: UIButton = FurnitureFeedCell.buttonForTitle("", imageName: "like")
