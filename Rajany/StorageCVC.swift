...
import UIKit
import Firebase
class StoragePost: SafeJsonObject {
    var item: String?
...
class StorageFeedController: UICollectionViewController, UICollectionViewDelegateFlowLayout {
    
    var posts = [StoragePost]()
    ...
    override func viewDidLoad() {
        ...
        let url = URL(string: AppConfig.storage_feed)
        URLSession.shared.dataTask(with: url!) { (data, response, error) in
            ...
            do {
                ...
                if let postsArray = json["storage"] as? [[String: String]] {
                    
                    self.posts = [StoragePost]()
        ...
        collectionView?.register(StorageFeedCell.self, forCellWithReuseIdentifier: cellId)
        setupNavBarButtons()
    ...
    func setupNavBarButtons() {
        ...
        let logo = UIBarButtonItem(image: UIImage(named: "logosmall")?.withRenderingMode(.alwaysOriginal), style: .plain, target: self, action: #selector(handleMore))
        let navItem = UINavigationItem(title: "Storage Ideas")
    ...
    override func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let StorageFeedCell = collectionView.dequeueReusableCell(withReuseIdentifier: cellId, for: indexPath) as! StorageFeedCell
        
        StorageFeedCell.post = posts[(indexPath as NSIndexPath).item]
        StorageFeedCell.feedController = self
        
        return StorageFeedCell
    }
...
class StorageFeedCell: UICollectionViewCell {
    var ref: FIRDatabaseReference!
    var feedController: StorageFeedController?
    ...
    let likeButton: UIButton = StorageFeedCell.buttonForTitle("", imageName: "like")
