...
import UIKit
import Firebase
class InteriorPost: SafeJsonObject {
    var item: String?
...
class InteriorFeedController: UICollectionViewController, UICollectionViewDelegateFlowLayout {
    
    var posts = [InteriorPost]()
    ...
    override func viewDidLoad() {
        ...
        let url = URL(string: AppConfig.interior_feed)
        URLSession.shared.dataTask(with: url!) { (data, response, error) in
            ...
            do {
                ...
                if let postsArray = json["interior"] as? [[String: String]]{
                    
                    self.posts = [InteriorPost]()
                    
                    for postDictionary in postsArray {
                        let post = InteriorPost()
        ...
        collectionView?.register(InteriorFeedCell.self, forCellWithReuseIdentifier: cellId)
        setupNavBarButtons()
    ...
    func setupNavBarButtons() {
        ...
        let logo = UIBarButtonItem(image: UIImage(named: "logosmall")?.withRenderingMode(.alwaysOriginal), style: .plain, target: self, action: #selector(handleMore))
        let navItem = UINavigationItem(title: "Interior Ideas")
    ...
    override func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let InteriorFeedCell = collectionView.dequeueReusableCell(withReuseIdentifier: cellId, for: indexPath) as! InteriorFeedCell
        
        InteriorFeedCell.post = posts[(indexPath as NSIndexPath).item]
        InteriorFeedCell.feedController = self
        
        return InteriorFeedCell
    }
...
class InteriorFeedCell: UICollectionViewCell {
    var ref: FIRDatabaseReference!
    var feedController: InteriorFeedController?
    
    var post: InteriorPost? {
        didSet {
    ...
    let likeButton: UIButton = InteriorFeedCell.buttonForTitle("", imageName: "like")
