...
import UIKit
import Firebase
class HintsPost: SafeJsonObject {
    var item: String?
...
class HintsFeedController: UICollectionViewController, UICollectionViewDelegateFlowLayout {
    ...
    override func viewDidLoad() {
        ...
        let url = URL(string: AppConfig.colr_hints_feed)
        URLSession.shared.dataTask(with: url!) { (data, response, error) in
            ...
            do {
                ...
                if let postsArray = json["hints"] as? [[String: String]] {
                    
                    self.posts = [HintsPost]()
        ...
        collectionView?.register(HintsFeedCell.self, forCellWithReuseIdentifier: cellId)
        setupNavBarButtons()
    ...
    func setupNavBarButtons() {
        ...
        let logo = UIBarButtonItem(image: UIImage(named: "logosmall")?.withRenderingMode(.alwaysOriginal), style: .plain, target: self, action: #selector(handleMore))
        let navItem = UINavigationItem(title: "Color Hints")
    ...
    override func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let HintsFeedCell = collectionView.dequeueReusableCell(withReuseIdentifier: cellId, for: indexPath) as! HintsFeedCell
        
        HintsFeedCell.post = posts[(indexPath as NSIndexPath).item]
        HintsFeedCell.feedController = self
        
        return HintsFeedCell
    }
...
class HintsFeedCell: UICollectionViewCell {
    var ref: FIRDatabaseReference!
    var feedController: HintsFeedController?
    ...
    let likeButton: UIButton = HintsFeedCell.buttonForTitle("", imageName: "like")
        ...
        if let user = FIRAuth.auth()?.currentUser {
            ...
            if username != nil {
                let idea = ["date": String(describing: date),
                            ...
                            "hint": String(describing: (self.post?.item)!),
                            "username": String(describing: username)]
                
                ref.child("colorhints").child(uid).childByAutoId().setValue(idea)
                let alert = UIAlertController(title: "Success!", message:"Added To Color Hints!", preferredStyle: .alert)
