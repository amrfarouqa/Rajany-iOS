...
import UIKit
import Firebase
class OutdoorPost: SafeJsonObject {
    var item: String?
...
class OutdoorFeedController: UICollectionViewController, UICollectionViewDelegateFlowLayout {
    
    var posts = [OutdoorPost]()
    ...
    override func viewDidLoad() {
        ...
        let url = URL(string: AppConfig.outdoor_feed)
        URLSession.shared.dataTask(with: url!) { (data, response, error) in
            ...
            do {
                ...
                if let postsArray = json["outdoor"] as? [[String: String]] {
                    
                    self.posts = [OutdoorPost]()
                    
                    for postDictionary in postsArray {
                        let post = OutdoorPost()
        ...
        collectionView?.register(OutdoorFeedCell.self, forCellWithReuseIdentifier: cellId)
        setupNavBarButtons()
    ...
    func setupNavBarButtons() {
        ...
        let logo = UIBarButtonItem(image: UIImage(named: "logosmall")?.withRenderingMode(.alwaysOriginal), style: .plain, target: self, action: #selector(handleMore))
        let navItem = UINavigationItem(title: "Outdoor Ideas")
    ...
    override func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let OutdoorFeedCell = collectionView.dequeueReusableCell(withReuseIdentifier: cellId, for: indexPath) as! OutdoorFeedCell
        
        OutdoorFeedCell.post = posts[(indexPath as NSIndexPath).item]
        OutdoorFeedCell.feedController = self
        
        return OutdoorFeedCell
    }
...
class OutdoorFeedCell: UICollectionViewCell {
    var ref: FIRDatabaseReference!
    var feedController: OutdoorFeedController?
    
    var post: OutdoorPost? {
        didSet {
    ...
    let likeButton: UIButton = OutdoorFeedCell.buttonForTitle("", imageName: "like")
