...
import UIKit
import Firebase
class IdeasPost: SafeJsonObject {
    var email: String?
    var username: String?
    var date: String?
    var idea: String?
...
class IdeasFeedController: UICollectionViewController, UICollectionViewDelegateFlowLayout {
    var posts = [IdeasPost]()
    ...
    override func viewDidLoad() {
        ...
        let ideasRef = FIRDatabase.database().reference().child("ideas").child(useruid!)
        
        ideasRef.observe(.childAdded, with: { (snapshot) in
           let post = IdeasPost()
            if let dictionary = snapshot.value as? NSDictionary {
                
                if let username = dictionary["username"] as? String, let email = dictionary["email"] as? String, let idea = dictionary["idea"] as? String, let date = dictionary["date"] as? String {
                    post.setValue(username as AnyObject?, forKey: "username")
                    post.setValue(email as AnyObject?, forKey: "email")
                    post.setValue(idea as AnyObject?, forKey: "idea")
        ...
        collectionView?.register(IdeasFeedCell.self, forCellWithReuseIdentifier: cellId)
        setupNavBarButtons()
    ...
    func setupNavBarButtons() {
        ...
        let navItem = UINavigationItem(title: "Ideabook")
    ...
    override func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let IdeasFeedCell = collectionView.dequeueReusableCell(withReuseIdentifier: cellId, for: indexPath) as! IdeasFeedCell
        
        IdeasFeedCell.post = posts[(indexPath as NSIndexPath).item]
        IdeasFeedCell.feedController = self
        
        return IdeasFeedCell
    }
...
class IdeasFeedCell: UICollectionViewCell {
    
    var feedController: IdeasFeedController?
    ...
    var post: IdeasPost? {
        didSet {
            ...
            if let statusImageName = post?.idea {
    ...
    func deleteItem(){
        let alert = UIAlertController(title: "Rajany", message:"Are You Sure You Want To Delete Idea?", preferredStyle: .alert)
