...
import UIKit
import Firebase
class VintagePost: SafeJsonObject {
    var item: String?
...
class VintageFeedController: UICollectionViewController, UICollectionViewDelegateFlowLayout {
    
    var posts = [VintagePost]()
    ...
    override func viewDidLoad() {
        ...
        let url = URL(string: AppConfig.vintage_feed)
        URLSession.shared.dataTask(with: url!) { (data, response, error) in
            ...
            do {
                ...
                if let postsArray = json["vintage"] as? [[String: String]] {
                    
                    self.posts = [VintagePost]()
                    
                    for postDictionary in postsArray {
                        let post = VintagePost()
        ...
        collectionView?.register(VintageFeedCell.self, forCellWithReuseIdentifier: cellId)
        setupNavBarButtons()
    ...
    func setupNavBarButtons() {
        ...
        let logo = UIBarButtonItem(image: UIImage(named: "logosmall")?.withRenderingMode(.alwaysOriginal), style: .plain, target: self, action: #selector(handleMore))
        let navItem = UINavigationItem(title: "Vintage Ideas")
    ...
    override func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let VintageFeedCell = collectionView.dequeueReusableCell(withReuseIdentifier: cellId, for: indexPath) as! VintageFeedCell
        
        VintageFeedCell.post = posts[(indexPath as NSIndexPath).item]
        VintageFeedCell.feedController = self
        
        return VintageFeedCell
    }
...
class VintageFeedCell: UICollectionViewCell {
    var ref: FIRDatabaseReference!
    var feedController: VintageFeedController?
    
    var post: VintagePost? {
        didSet {
    ...
    let likeButton: UIButton = VintageFeedCell.buttonForTitle("", imageName: "like")
