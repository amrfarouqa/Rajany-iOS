...
import UIKit
import Firebase
class SchemesVPost: SafeJsonObject {
    var email: String?
    var username: String?
    var date: String?
    var scheme: String?
...
class SchemesVFeedController: UICollectionViewController, UICollectionViewDelegateFlowLayout {
    var posts = [SchemesVPost]()
    ...
    override func viewDidLoad() {
        ...
        let ideasRef = FIRDatabase.database().reference().child("colorschemes").child(useruid!)
        
        ideasRef.observe(.childAdded, with: { (snapshot) in
            let post = SchemesVPost()
            if let dictionary = snapshot.value as? NSDictionary {
                
                if let username = dictionary["username"] as? String, let email = dictionary["email"] as? String, let scheme = dictionary["scheme"] as? String, let date = dictionary["date"] as? String {
                    post.setValue(username as AnyObject?, forKey: "username")
                    post.setValue(email as AnyObject?, forKey: "email")
                    post.setValue(scheme as AnyObject?, forKey: "scheme")
        ...
        collectionView?.register(SchemesVFeedCell.self, forCellWithReuseIdentifier: cellId)
        setupNavBarButtons()
    ...
    func setupNavBarButtons() {
        ...
        let navItem = UINavigationItem(title: "Schemes Viewer")
    ...
    override func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let SchemesVFeedCell = collectionView.dequeueReusableCell(withReuseIdentifier: cellId, for: indexPath) as! SchemesVFeedCell
        
        SchemesVFeedCell.post = posts[(indexPath as NSIndexPath).item]
        SchemesVFeedCell.feedController = self
        
        return SchemesVFeedCell
    }
...
class SchemesVFeedCell: UICollectionViewCell {
    
    var feedController: SchemesVFeedController?
    
    var post: SchemesVPost? {
        didSet {
            ...
            if let statusImageName = post?.scheme {
    ...
    func deleteItem(){
        let alert = UIAlertController(title: "Rajany", message:"Are You Sure You Want To Delete Color Scheme?", preferredStyle: .alert)
