...
import UIKit
import Firebase
class BedroomPost: SafeJsonObject {
    var item: String?
...
class BedroomFeedController: UICollectionViewController, UICollectionViewDelegateFlowLayout {
    
    var posts = [BedroomPost]()
    ...
    override func viewDidLoad() {
        ...
        let url = URL(string: AppConfig.bedroom_feed)
        URLSession.shared.dataTask(with: url!) { (data, response, error) in
            ...
            do {
                ...
                if let postsArray = json["bedroom"] as? [[String: String]] {
                    
                    self.posts = [BedroomPost]()
                    
                    for postDictionary in postsArray {
                        let post = BedroomPost()
        ...
        collectionView?.register(BedroomFeedCell.self, forCellWithReuseIdentifier: cellId)
        setupNavBarButtons()
    ...
    func setupNavBarButtons() {
        ...
        let logo = UIBarButtonItem(image: UIImage(named: "logosmall")?.withRenderingMode(.alwaysOriginal), style: .plain, target: self, action: #selector(handleMore))
        let navItem = UINavigationItem(title: "Bedroom Ideas")
    ...
    override func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let BedroomFeedCell = collectionView.dequeueReusableCell(withReuseIdentifier: cellId, for: indexPath) as! BedroomFeedCell
        
        BedroomFeedCell.post = posts[(indexPath as NSIndexPath).item]
        BedroomFeedCell.feedController = self
        
        return BedroomFeedCell
    }
...
class BedroomFeedCell: UICollectionViewCell {
    var ref: FIRDatabaseReference!
    var feedController: BedroomFeedController?
    
    var post: BedroomPost? {
        didSet {
    ...
    let likeButton: UIButton = BedroomFeedCell.buttonForTitle("", imageName: "like")
