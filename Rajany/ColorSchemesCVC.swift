...
import UIKit
import Firebase
class SchemesPost: SafeJsonObject {
    var item: String?
...
class SchemesFeedController: UICollectionViewController, UICollectionViewDelegateFlowLayout {
    
    var posts = [SchemesPost]()
    ...
    override func viewDidLoad() {
        ...
        let url = URL(string: AppConfig.color_schemes_feed)
        URLSession.shared.dataTask(with: url!) { (data, response, error) in
            ...
            do {
                ...
                if let postsArray = json["schemes"] as? [[String: String]] {
                    
                    self.posts = [SchemesPost]()
                    
                    for postDictionary in postsArray {
                        let post = SchemesPost()
        ...
        collectionView?.register(SchemesFeedCell.self, forCellWithReuseIdentifier: cellId)
        setupNavBarButtons()
    ...
    func setupNavBarButtons() {
        ...
        let logo = UIBarButtonItem(image: UIImage(named: "logosmall")?.withRenderingMode(.alwaysOriginal), style: .plain, target: self, action: #selector(handleMore))
        let navItem = UINavigationItem(title: "Color Schemes")
    ...
    override func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let SchemesFeedCell = collectionView.dequeueReusableCell(withReuseIdentifier: cellId, for: indexPath) as! SchemesFeedCell
        
        SchemesFeedCell.post = posts[(indexPath as NSIndexPath).item]
        SchemesFeedCell.feedController = self
        
        return SchemesFeedCell
    }
...
class SchemesFeedCell: UICollectionViewCell {
    var ref: FIRDatabaseReference!
    var feedController: SchemesFeedController?
    
    var post: SchemesPost? {
        didSet {
    ...
    let likeButton: UIButton = SchemesFeedCell.buttonForTitle("", imageName: "like")
        ...
        if let user = FIRAuth.auth()?.currentUser {
            ...
            if username != nil {
                let idea = ["date": String(describing: date),
                            ...
                            "scheme": String(describing: (self.post?.item)!),
                            "username": String(describing: username)]
                
                ref.child("colorschemes").child(uid).childByAutoId().setValue(idea)
                let alert = UIAlertController(title: "Success!", message:"Added To Color Schemes!", preferredStyle: .alert)
