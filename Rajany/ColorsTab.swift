...
class ColorsController: UICollectionViewController, UICollectionViewDelegateFlowLayout {
    ...
    override func viewDidLoad() {
        ...
        if Reachability.isConnectedToNetwork() == true
        {
            ...
            let url = URL(string: AppConfig.colors_feed)
            ...
            collectionView?.register(ColorsFeedCell.self, forCellWithReuseIdentifier: cellId)
    ...
    func setupNavBarButtons2() {
        ...
        let colorSchemes = UIBarButtonItem(image: UIImage(named: "brushsmall")?.withRenderingMode(.alwaysOriginal), style: .plain, target: self, action: #selector(handleShowColorSchemes))
        
        let colorPicks = UIBarButtonItem(image: UIImage(named: "pensmall")?.withRenderingMode(.alwaysOriginal), style: .plain, target: self, action: #selector(handleShowColorPicks))
        
        let colorHints = UIBarButtonItem(image: UIImage(named: "paintsmall")?.withRenderingMode(.alwaysOriginal), style: .plain, target: self, action: #selector(handleShowColorHints))
    
        let navItem = UINavigationItem(title: "")
        
        navItem.rightBarButtonItems = [colorSchemes, colorPicks, colorHints]
        navBar.setItems([navItem], animated: false)
        
        self.view.addSubview(navBar);
    }
    
    func handleShowColorPicks() {
        let ColorPicker = self.storyboard!.instantiateViewController(withIdentifier: "ColorPicker")
        let appDelegate = UIApplication.shared.delegate as! AppDelegate
        appDelegate.window!.rootViewController = ColorPicker
        appDelegate.window!.makeKeyAndVisible()
    }
    
    func handleShowColorSchemes() {
        let ColorSchemes = self.storyboard!.instantiateViewController(withIdentifier: "SchemesFeedController")
        let appDelegate = UIApplication.shared.delegate as! AppDelegate
        appDelegate.window!.rootViewController = ColorSchemes
        appDelegate.window!.makeKeyAndVisible()

    }
    
    func handleShowColorHints() {
        let ColorHints = self.storyboard!.instantiateViewController(withIdentifier: "HintsFeedController")
        let appDelegate = UIApplication.shared.delegate as! AppDelegate
        appDelegate.window!.rootViewController = ColorHints
        appDelegate.window!.makeKeyAndVisible()
    ...
    override func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let ColorsFeedCell = collectionView.dequeueReusableCell(withReuseIdentifier: cellId, for: indexPath) as! ColorsFeedCell
        
        ColorsFeedCell.post = posts[(indexPath as NSIndexPath).item]
        ColorsFeedCell.feedController = self
        
        return ColorsFeedCell
    }
...
class ColorsFeedCell: UICollectionViewCell {
    
    var feedController: ColorsController?
    ...
    let likeButton = ColorsFeedCell.buttonForTitle("", imageName: "")
    let commentButton: UIButton = ColorsFeedCell.buttonForTitle("Share", imageName: "share")
    let shareButton: UIButton = ColorsFeedCell.buttonForTitle("", imageName: "")
    ...
    func setupViews() {
        ...
        statusImageView.addGestureRecognizer(UITapGestureRecognizer(target: self, action: #selector(ColorsFeedCell.animate as (ColorsFeedCell) -> () -> ())))
