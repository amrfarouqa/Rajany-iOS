...
import UIKit
import Firebase
class RestaurantPost: SafeJsonObject {
    var item: String?
...
class RestaurantFeedController: UICollectionViewController, UICollectionViewDelegateFlowLayout {
    
    var posts = [RestaurantPost]()
    ...
    override func viewDidLoad() {
        ...
        let url = URL(string: AppConfig.restaurant_feed)
        URLSession.shared.dataTask(with: url!) { (data, response, error) in
            ...
            do {
                ...
                if let postsArray = json["restaurant"] as? [[String: String]] {
                    
                    self.posts = [RestaurantPost]()
                    
                    for postDictionary in postsArray {
                        let post = RestaurantPost()
        ...
        collectionView?.register(RestaurantFeedCell.self, forCellWithReuseIdentifier: cellId)
        setupNavBarButtons()
    ...
    func setupNavBarButtons() {
        ...
        let logo = UIBarButtonItem(image: UIImage(named: "logosmall")?.withRenderingMode(.alwaysOriginal), style: .plain, target: self, action: #selector(handleMore))
        let navItem = UINavigationItem(title: "Restaurant Ideas")
    ...
    override func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let RestaurantFeedCell = collectionView.dequeueReusableCell(withReuseIdentifier: cellId, for: indexPath) as! RestaurantFeedCell
        
        RestaurantFeedCell.post = posts[(indexPath as NSIndexPath).item]
        RestaurantFeedCell.feedController = self
        
        return RestaurantFeedCell
    }
...
class RestaurantFeedCell: UICollectionViewCell {
    var ref: FIRDatabaseReference!
    var feedController: RestaurantFeedController?
    
    var post: RestaurantPost? {
        didSet {
    ...
    let likeButton: UIButton = RestaurantFeedCell.buttonForTitle("", imageName: "like")
