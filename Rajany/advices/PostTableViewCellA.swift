...
@objc(PostTableViewCellA)
class PostTableViewCellA: UITableViewCell {
  ...
  @IBAction func didTapStarButton(_ sender: AnyObject) {
    /*if let postKey = postKey {
      postRef = FIRDatabase.database().reference().child("stories").child("posts").child(postKey)
      incrementStars(forRef: postRef)
      postRef.observeSingleEvent(of: .value, with: { (snapshot) in
        let value = snapshot.value as? NSDictionary
        if let uid = value?["uid"] as? String {
          let userPostRef = FIRDatabase.database().reference().child("stories")
            .child("user-posts")
            .child(uid)
            .child(postKey)
