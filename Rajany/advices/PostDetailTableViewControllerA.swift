...
@objc(PostDetailTableViewControllerA)
class PostDetailTableViewControllerA: UITableViewController, UITextFieldDelegate {
  ...
  override func viewDidLoad() {
    ...
    let nib = UINib(nibName: "PostTableViewCellA", bundle: nil)
  ...
  override func viewWillDisappear(_ animated: Bool) {
    ...
    if let uid = FIRAuth.auth()?.currentUser?.uid {
      FIRDatabase.database().reference().child("advice").child("users").child(uid).removeAllObservers()
    }
  }
  ...
  override func tableView(_ tableView: UITableView,
    ...
    case kSectionPost:
      ...
      if (FIRAuth.auth()?.currentUser?.uid) != nil {
        guard let postcell = cell as? PostTableViewCellA else {
          break
        }
