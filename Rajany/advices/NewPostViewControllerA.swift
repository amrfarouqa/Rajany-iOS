...
@objc(NewPostViewControllerA)
class NewPostViewControllerA: UIViewController, UITextFieldDelegate {
  ...
  override func viewDidLoad() {
    ...
    self.ref = FIRDatabase.database().reference().child("advice")
  ...
  @IBAction func didTapShare(_ sender: AnyObject) {
    ...
    if let usernamee = FIRAuth.auth()?.currentUser?.displayName {
        ref.child("users").child(userID!).observeSingleEvent(of: .value, with: { (snapshot) in
            // Get user value
            let value = snapshot.value as? NSDictionary
            let username = value?["username"] as? String ?? ""
            _ = User.init(username: username)
            
            // [START_EXCLUDE]
            // Write new post
            self.writeNewPost(withUserID: userID!, username: usernamee, title: self.titleTextField.text!, body: self.bodyTextView.text)
        ...
        ref.child("users").child(userID!).observeSingleEvent(of: .value, with: { (snapshot) in
            ...
            // [START_EXCLUDE]
            // Write new post
            self.writeNewPost(withUserID: userID!, username: (FIRAuth.auth()?.currentUser?.email)!, title: self.titleTextField.text!, body: self.bodyTextView.text)
