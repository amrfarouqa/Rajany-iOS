...
import UIKit
import Firebase
class KitchenPost: SafeJsonObject {
    var item: String?
...
class KitchenFeedController: UICollectionViewController, UICollectionViewDelegateFlowLayout {
    
    var posts = [KitchenPost]()
    ...
    override func viewDidLoad() {
        ...
        let url = URL(string: AppConfig.kitchen_feed)
        URLSession.shared.dataTask(with: url!) { (data, response, error) in
            ...
            do {
                ...
                if let postsArray = json["kitchen"] as? [[String: String]] {
                    
                    self.posts = [KitchenPost]()
                    
                    for postDictionary in postsArray {
                        let post = KitchenPost()
        ...
        collectionView?.register(KitchenFeedCell.self, forCellWithReuseIdentifier: cellId)
        setupNavBarButtons()
    ...
    func setupNavBarButtons() {
        ...
        let logo = UIBarButtonItem(image: UIImage(named: "logosmall")?.withRenderingMode(.alwaysOriginal), style: .plain, target: self, action: #selector(handleMore))
        let navItem = UINavigationItem(title: "Kitchen Ideas")
    ...
    override func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let KitchenFeedCell = collectionView.dequeueReusableCell(withReuseIdentifier: cellId, for: indexPath) as! KitchenFeedCell
        
        KitchenFeedCell.post = posts[(indexPath as NSIndexPath).item]
        KitchenFeedCell.feedController = self
        
        return KitchenFeedCell
    }
...
class KitchenFeedCell: UICollectionViewCell {
    var ref: FIRDatabaseReference!
    var feedController: KitchenFeedController?
    
    var post: KitchenPost? {
        didSet {
    ...
    let likeButton: UIButton = KitchenFeedCell.buttonForTitle("", imageName: "like")
