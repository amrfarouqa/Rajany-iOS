...
import UIKit
import Firebase
class FireplacePost: SafeJsonObject {
    var item: String?
...
class FireplaceFeedController: UICollectionViewController, UICollectionViewDelegateFlowLayout {
    
    var posts = [FireplacePost]()
    ...
    override func viewDidLoad() {
        ...
        let url = URL(string: AppConfig.fireplace_feed)
        URLSession.shared.dataTask(with: url!) { (data, response, error) in
            ...
            do {
                ...
                if let postsArray = json["fireplace"] as? [[String: String]] {
                    
                    self.posts = [FireplacePost]()
                    
                    for postDictionary in postsArray {
                        let post = FireplacePost()
        ...
        collectionView?.register(FireplaceFeedCell.self, forCellWithReuseIdentifier: cellId)
        setupNavBarButtons()
    ...
    func setupNavBarButtons() {
        ...
        let logo = UIBarButtonItem(image: UIImage(named: "logosmall")?.withRenderingMode(.alwaysOriginal), style: .plain, target: self, action: #selector(handleMore))
        let navItem = UINavigationItem(title: "Fireplace Ideas")
    ...
    override func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let FireplaceFeedCell = collectionView.dequeueReusableCell(withReuseIdentifier: cellId, for: indexPath) as! FireplaceFeedCell
        
        FireplaceFeedCell.post = posts[(indexPath as NSIndexPath).item]
        FireplaceFeedCell.feedController = self
        
        return FireplaceFeedCell
    }
...
class FireplaceFeedCell: UICollectionViewCell {
    var ref: FIRDatabaseReference!
    var feedController: FireplaceFeedController?
    
    var post: FireplacePost? {
        didSet {
    ...
    let likeButton: UIButton = FireplaceFeedCell.buttonForTitle("", imageName: "like")
